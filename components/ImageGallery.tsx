      ...
      >
        {images.map((img, idx) => {
          ...
          if (count === 1) {
            ...
            if (idx === 0)
              mosaicClass =
                "md:col-span-2 md:row-span-2 aspect-[4/3] md:aspect-auto h-full shadow-md";
          ...
          return (
            <div
              key={idx}
              className={`relative overflow-hidden group ${mosaicClass}`}
            >
              <img
                src={img}
      ...
      {caption && (
        <figcaption className="mt-4 flex items-start justify-center gap-2 text-center px-4">
          ...
          <span className="font-serif text-sm md:text-base text-ink/60 italic leading-relaxed max-w-2xl">
            <span className="text-accent not-italic font-bold text-xs tracking-widest uppercase mr-2">
              Figura.
            </span>
            {caption}
          </span>
