...
export const ArticleDetail: FC = () => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    if (article) {
      document.title = `${article.title} | Tempo de Partir`;
    }
  }, [id, article]);
            ...
            <div className="space-y-10 text-[19px] md:text-[21px] leading-[1.8] text-justify font-light text-ink/95">
              {remainingParagraphs.map((paragraph, index) => {
                ...
                if (trimmed.startsWith("GALLERY:")) {
                  const [urlsPart, caption] = trimmed
                    .replace("GALLERY:", "")
                    .split("|");
                  const urls = urlsPart
                    ? urlsPart.split(",").map((u) => u.trim())
                    : [];
                  return (
                    <ImageGallery
                      key={index}
                ...
                if (trimmed.startsWith("##"))
                  return (
                    <h2
                      key={index}
                      className="font-serif text-3xl md:text-4xl text-ink mt-16 mb-8 border-l-4 border-gold pl-6 leading-tight"
                    >
                      {trimmed.replace(/^##\s?/, "")}
                    </h2>
                  );
