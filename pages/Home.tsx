...
export const Home: FC = () => {
  ...
  const [mainHeroTitle, mainHeroSubtitle] = getMainTitleParts(
    mainArticle.title
  );
  ...
  return (
    <div className="animate-fade-in">
      <section className="flex flex-col lg:flex-row min-h-[calc(100vh-6rem)]">
        <div className="relative w-full lg:w-[70%] min-h-[70vh] lg:min-h-full group overflow-hidden flex flex-col">
          ...
          <div className="relative flex-1 flex flex-col justify-end p-8 md:p-16 lg:p-24 max-w-6xl">
            ...
            <h1 className="font-serif text-paper mb-6 md:mb-8 leading-tight drop-shadow-xl max-w-3xl">
              ...
              {mainHeroSubtitle && (
                <span className="block text-xl md:text-3xl font-light italic opacity-90">
                  {mainHeroSubtitle.trim()}
                </span>
              )}
            </h1>
            ...
            >
              <span className="border-b border-white group-hover/link:border-accent pb-2 transition-colors">
                Ler Ensaio Completo
              </span>
        ...
        <div className="w-full lg:w-[30%] bg-paper flex flex-col border-l border-earth/10 relative z-10">
          <div className="flex-1 overflow-y-auto custom-scrollbar pt-8">
            {sidebarArticles.map((article) => {
              ...
              const [sideTitle, sideSubtitle] = getMainTitleParts(
                article.title
              );
              return (
                <div
                ...
                >
                  <div className="p-8 flex gap-6 items-start">
                    <div className="flex-1 flex flex-col">
                      <div className="flex justify-between items-center mb-3">
                        <span
                          className={`text-xs font-bold uppercase tracking-[0.2em] transition-colors ${
                            isExpanded
                              ? "text-earth"
                              : "text-earth group-hover:text-paper"
                          }`}
                        >
                          {article.category}
                        </span>
                        {isExpanded ? (
                          ...
                          <Plus
                            className={`h-3 w-3 text-earth/60 ${
                              isExpanded ? "" : "group-hover:text-paper"
                            }`}
                            strokeWidth={1}
                          />
                        )}
                      ...
                      <h3
                        className={`font-serif leading-tight mb-2 transition-colors duration-300 ${
                          isExpanded
                            ? "text-accent"
                      ...
                      >
                        ...
                        {sideSubtitle && (
                          <span className="block text-base font-light italic mt-1 opacity-90">
                            {sideSubtitle.trim()}
                          </span>
                        )}
                      </h3>
                      <div
                        className={`overflow-hidden transition-all duration-700 ease-in-out ${
                          isExpanded
                            ? "max-h-96 opacity-100 mt-5 mb-4"
                            : "max-h-0 opacity-0"
                        }`}
                      >
                        <p className="font-sans text-base text-ink leading-relaxed mb-6 font-normal">
                          {article.excerpt}
                        </p>
                        <Link
                      ...
                      <span
                        className={`text-[9px] font-sans mt-3 block uppercase tracking-widest ${
                          isExpanded
                            ? "text-earth/80"
                            : "text-earth/70 group-hover:text-paper/70"
                        }`}
                      >
                        {article.date}
                      </span>
          ...
          <Link
            to="/editorial"
            className="p-8 bg-ink text-paper group hover:bg-ink/90 transition-colors flex justify-between items-center mt-auto"
          >
            <span className="font-serif text-xl italic pl-2 group-hover:pl-4 transition-all duration-500">
              Explorar Editorial
            </span>
            <ArrowRight
              className="h-5 w-5 text-gold transform group-hover:translate-x-2 transition-transform duration-500"
              strokeWidth={1}
            />
          </Link>
        </div>
      ...
      <section className="py-16 md:py-32 bg-accent px-4 border-t border-white/10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h2 className="font-serif text-3xl md:text-6xl text-paper leading-tight mb-8 md:mb-12 font-medium">
            Viajar é mudar de ritmo, <br />
            não apenas de lugar.
          </h2>
          <div className="w-px h-12 md:h-16 bg-gold mx-auto mb-8 md:mb-12" />
          <p className="font-sans text-paper/95 text-lg md:text-xl leading-[1.8] mb-10 font-light max-w-2xl mx-auto">
            Acreditamos que as memórias mais bonitas são aquelas que a gente
            constrói no nosso próprio tempo.
          </p>
          <Link
            to="/manifesto"
            className="inline-block px-8 py-4 md:px-12 md:py-5 border-[0.5px] border-paper text-paper hover:bg-paper hover:text-accent transition-all duration-500 text-[10px] uppercase tracking-[0.2em] font-bold"
          >
            Ler Manifesto
          </Link>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-10 md:mb-20">
          <div className="text-center">
            <h3 className="font-serif text-3xl md:text-4xl text-ink mb-4">
              Caminhos para a sua próxima jornada
            </h3>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-earth/10 border-y border-earth/10">
            {[
              ...
              <Link
                to="/editorial"
                key={idx}
                className={`${cat.bg} ${cat.hoverBg} py-10 px-6 md:p-12 flex flex-col items-center justify-center text-center group cursor-pointer transition-colors duration-700`}
              >
                <cat.icon
                  strokeWidth={0.5}
                  className={`h-10 w-10 md:h-12 md:w-12 mb-6 md:mb-8 ${cat.textColor} ${cat.hoverText}`}
                />
                <h3
                  className={`font-serif text-2xl md:text-3xl mb-4 md:mb-6 ${cat.textColor} ${cat.hoverText}`}
                >
                  {cat.title}
                </h3>
                <p
                  className={`font-sans text-xs font-medium tracking-[0.1em] uppercase opacity-80 ${cat.textColor} ${cat.hoverText}`}
                >
                  {cat.desc}
                </p>
                <div
                  className={`mt-6 md:mt-10 opacity-0 group-hover:opacity-100 transition-all duration-700 transform translate-y-4 group-hover:translate-y-0 ${cat.hoverText}`}
                >
                  <ArrowRight
